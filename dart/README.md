...
> **Note:** the `servirtium` package is **not published to pub.dev yet**, so
> `dart pub add --dev servirtium` does **not** give you this library. Get the
...
```sh
# from the dart binding directory, with libservirtium_vcr-v0.1.0-linux-x86_64.so downloaded:
mkdir -p lib/native
cp libservirtium_vcr-v0.1.0-linux-x86_64.so lib/native/libservirtium_vcr.so
