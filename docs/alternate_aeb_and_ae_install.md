...
The pins below (`AE_PIN=0.698.0`, `AEB_REF=v0.319`) mirror this repo's
...
AE_PIN=0.698.0 AEB_REF=v0.319 aeb_bootstrap        # ensures ae (>= AE_PIN) THEN aeb
```
...
| Var | Meaning |
|---|---|
| `AE_PIN` | The one `ae` this repo is verified against (`0.698.0`) — deliberately equal to `AE_FETCH`, not a permissive floor; see the pin note atop `bootstrap.sh`. In sourced mode `ae_ensure` requires it. |
...
AE_PIN=0.698.0 AEB_REF=v0.319 AEB_FROM_SOURCE=1 bash get.sh
```
