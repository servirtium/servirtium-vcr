<!--
NEEDS REWORK (2.0): this is a 1.x reference recording (Servirtium markdown
tape) for the todo-backend compatibility suite. The tape FORMAT is unchanged
in 2.0, so this recording is still valid input, but the suite harness that
drove it (the deleted standalone proxy server) is gone. Re-wire it against the
new servirtium.playback / servirtium.record API. See MIGRATION.md and
COMPATIBILITY_SUITE.md. (HTML comment so the tape still parses.)
-->
