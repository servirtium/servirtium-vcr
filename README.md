...
- **Installing `ae` + `aeb`** needs only `curl` — as of aeb v0.307 the toolchain
...
`v0.650.0`), and the aeb floor is `>= 0.307` (v0.298 made the bundle installer make-free; v0.300 aligned the release asset on `x86_64`; v0.307 is current). The commands below pin that
...
file: `curl … -o get.sh && AE_PIN=0.650.0 AEB_REF=v0.307 bash get.sh` — same
result.)
...
curl -fsSL https://raw.githubusercontent.com/aether-lang-dev/aeo/main/get.sh \
  | AE_PIN=0.650.0 AEB_REF=v0.307 AEO_REF=v0.2.3 sh
```
...
# or, with ae (>= AE_PIN) and aeb (>= 0.307) already on PATH:
