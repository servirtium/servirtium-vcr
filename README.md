...
- **Installing `ae` + `aeb`** needs only `curl` — as of aeb v0.300 the toolchain
...
`v0.650.0`), and the aeb floor is `>= 0.300` (v0.298 made the bundle installer make-free; v0.300 is current, and aligns the release asset on the `x86_64` arch word). The commands below pin that
...
# or, with ae (>= AE_PIN) and aeb (>= 0.300) already on PATH:
