...
number** — `0.706.0`, the single ae this repo is *verified* against. (They used
...
`AEB_REF` == floor == `v0.324`, the one aeb this repo is verified against — and
...
file: `curl … -o get.sh && AE_PIN=0.706.0 AEB_REF=v0.324 bash get.sh` — same
result.)
...
curl -fsSL https://raw.githubusercontent.com/aether-lang-dev/aeo/main/get.sh \
  | AE_PIN=0.706.0 AEB_REF=v0.324 AEO_REF=v0.2.3 sh
```
