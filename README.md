...
- **Installing `ae` + `aeb`** needs only `curl` — as of aeb v0.311 the toolchain
...
number** — `0.675.0`, the single ae this repo is *verified* against. (They used
...
file: `curl … -o get.sh && AE_PIN=0.675.0 AEB_REF=v0.311 bash get.sh` — same
result.)
...
curl -fsSL https://raw.githubusercontent.com/aether-lang-dev/aeo/main/get.sh \
  | AE_PIN=0.675.0 AEB_REF=v0.311 AEO_REF=v0.2.3 sh
```
