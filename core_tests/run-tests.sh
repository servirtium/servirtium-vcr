...
# hand-written core/aether_vcr.c via --extra, producing a test binary whose
...
for t in core_tests/test_vcr_*.ae; do
  n="$(basename "$t" .ae)"
  if ae build "$t" --extra core/aether_vcr.c -o "target/core_tests/$n" >"target/core_tests/$n.log" 2>&1 \
