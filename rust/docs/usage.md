...
`.match_multiple()` matches any recorded interaction (order-independent) and
does not consume it, for polling/retries or non-deterministic request order,
while `.match_header(name)` matches on just that one named request header's
value and ignores the rest of the recorded header block.
