...
final class RecordBuilder extends VcrBuilderBase
{
    ...
    protected function applyConfig(): void
    {
        ...
        if ($this->indentCodeBlocksOn) {
            $lib->aether_vcr_embed_indent_code_blocks();
        }
        if ($this->emphasizeHttpVerbsOn) {
            $lib->aether_vcr_embed_emphasize_http_verbs();
        }
        foreach ($this->redactions as [$field, $pattern, $replacement]) {
            self::check($lib->aether_vcr_embed_redact($field->value, $pattern, $replacement), 'redact');
        }
    ...
    public function start(): VcrServer
    {
        ...
        $this->applyConfig();
        $lib = Native::lib();
        $handle = $lib->aether_vcr_embed_start_record(
            $this->labelValue,
            $this->tapePath,
            $this->upstreamBase,
            $this->hostValue,
        ...
        if ($handle === null || FFI::isNull($handle)) {
            throw new VcrException(
                ...
                . "(upstream '{$this->upstreamBase}'): " . self::drainStartError()
            );
        }
        ...
        if ($this->note !== null) {
            ...
            self::check($lib->aether_vcr_embed_note($title, $body), 'note');
        }
